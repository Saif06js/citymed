/* =============================================
   ...
   const STRIPE_PK = 'pk_test_51TT1yqLCtgIKuIxliEDXxDeTZ2clIxGklCYBPHCKRE7JqAzm05PFoq8joBt3bQX17GlBUpXI9Idlf8gmcAe4ZdLx00cl5iPOp1';
   ...
   ['closeLogin','closeRegister','closeHelp','closeHospital','closeBooking','closeProfile','closeReports','closeForgot','closeQr','closeOtp'].forEach(id => {     document.getElementById(id)?.addEventListener('click', closeAllModals);
   });
   ...
   window.handleRegister = async function() {
    ...
    document.getElementById('otpNewPassGroup').style.display = 'none';
   ...
   window.sendResetCode = async function() {
    ...
    window._otpEmail = email;
    window._otpType  = 'recovery';
  
    switchModal('forgotModal', 'otpModal');
    ...
    document.getElementById('otpNewPassGroup').style.display = 'block';
    document.getElementById('otpError').textContent = '';
    clearOtpBoxes();
    document.querySelector('.otp-box')?.focus();
  };
  ...
  stripeCardElement = elements.create('card', {
    style: {
      base: {
        ...
        fontSize: '14px',
        color: '#0f1f2e',
        '::placeholder': { color: '#7a90a8' }
      }
    }
  });
  const cardContainer = document.getElementById('stripeCardContainer');
    ...
    try {
      ...
      const res = await fetch(`https://qzppzysjzgslfmlalqcd.supabase.co/functions/v1/create-payment`, {
        ...
        body: JSON.stringify({
          ...
          bookingDetails: { name, hospital, department: dept, date }
        })
      });
      ...
      await db.from('bookings').insert({
        ...
        full_name: name, phone, hospital,
        department: dept, book_date: date, book_time: time,
      ...
      bookingGoStep(1);
      selectedDoctor = null;
      showToast(`Payment successful! Appointment confirmed for ${name} on ${date} 🎉`, 'success', 6000);
...
// Auto-jump between boxes + paste support
...
window.verifyOtp = async function() {
  ...
  const { error } = await db.auth.verifyOtp({ email, token: otp, type });
  ...
  if (type === 'signup') {
    closeAllModals();
    showToast('Email verified! Welcome to CityMed 🎉', 'success', 5000);
  } else if (type === 'recovery') {
    const newPass = document.getElementById('otpNewPass')?.value;
    if (!newPass || newPass.length < 8) {
      errEl.textContent = 'Enter a new password (min. 8 characters).';
      return;
    }
    const { error: passErr } = await db.auth.updateUser({ password: newPass });
    if (passErr) { errEl.textContent = passErr.message; return; }
    closeAllModals();
...
window.verifyOtp = async function() {
  const otp   = getOtpValue();
  const email = window._otpEmail;
  const type  = window._otpType; // 'signup' or 'recovery'
  const errEl = document.getElementById('otpError');
  errEl.textContent = '';
